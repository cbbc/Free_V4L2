...
static int OrangePi_Format_Support(struct OrangePi_v4l2_device *dev)
{
    ...
    if(ioctl(dev->fd,VIDIOC_TRY_FMT,&fmt) == -1) {
	printf("Don't support this format!\n");
	return -1;
    } else
...
static int OrangePi_Capture(struct OrangePi_v4l2_device *dev)
{
    ...
    fwrite(dev->buffers->Raw_buffers[buf.index].start, 
	    dev->buffers->Raw_buffers[buf.index].length , 1 , dev->buffers->Out_fd);
